<script>
  ...
  // Reactive statement to update description when readMore changes
  $: if (readMore) {
    updateDescription();
  }
...
<div on:click={() => { readMore = !readMore }} class="flex items-center gap-x-3 border-b hover:bg-base-200 px-4 cursor-pointer">
  <div class="py-4 w-full">
    ...
    {:else}
      <p bind:this={descriptionEle} class="text-sm pt-1 whitespace-pre-wrap"></p>
    {/if}
  </div>

  {#if !archived}
    <button on:click={() => { selectedId.set(id) }} onclick="archive_confirmation_modal.showModal()" class="btn btn-square bg-base-100">
      <Icon src={ArchiveBox} size="20" />
    </button>
  {:else}
    <button on:click={() => { selectedId.set(id) }} onclick="unarchive_confirmation_modal.showModal()" class="btn btn-square bg-base-100">
      <Icon src={ArchiveBoxArrowDown} size="20" />
    </button>
  {/if}
</div>
